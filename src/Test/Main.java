...
public class Main {
	...
	static public void game() {
		...
		{
			if (card.color.equals(playPile.getTop().color)||card.color=="wild"||card.ID.equals(playPile.getTop().ID)) {
	...
	static class ActionMenu {
		...
		static void allowAction(String action, Card card) {
			if (action.equals("play")) {
				...
				allowedActions.add(() -> {
					 int i = 0;;
					while (i<currPlayer.hand.numberOfCards()) {
						if (currPlayer.hand.cards.get(i)==card) {
